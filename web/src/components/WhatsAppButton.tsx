import { isWhatsAppPlaceholder, whatsAppHref } from "@/lib/contact"
import { useCookieChoice } from "@/lib/cookieConsent"

/**
 * Floating WhatsApp entry point. It is a plain outbound link, sets no cookies
 * and does not depend on the cookie-consent choice, so it is always visible.
 * While the consent bar is showing, the button lifts above it.
 */
export function WhatsAppButton() {
  const choice = useCookieChoice()
  if (isWhatsAppPlaceholder) return null
  const barVisible = choice === null
  ...
  return (
    <a
      ...
      style={{
        bottom: `calc(env(safe-area-inset-bottom, 0px) + ${barVisible ? "var(--cookie-bar-h, 4.5rem) + 1rem" : "1rem"})`,
        right: "calc(env(safe-area-inset-right, 0px) + 1rem)",
        transition: "bottom 0.25s ease",
      }}
      className="fixed z-50 inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg shadow-[#25D366]/40 hover:scale-105 lg:!right-7"
    >
