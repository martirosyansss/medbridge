...
export function Specialties() {
  ...
  return (
    <section id="specialties" className="bg-bone py-24 lg:py-32">
      <div className="mx-auto max-w-content px-6 lg:px-10">
        ...
        <div className="reveal mt-12 flex flex-wrap items-center gap-3">
          <div role="tablist" aria-label="Filter specialties" className="flex flex-wrap gap-2">
            {FILTERS.map((f) => (
              <button
                key={f.key}
                type="button"
          ...
          <div className="ml-auto relative w-full sm:w-72">
            <input
              ...
              className="w-full rounded-sm border border-ink/15 bg-paper px-4 py-2.5 pr-10 text-sm placeholder:text-ink/40 focus:border-claret focus:outline-none focus:ring-2 focus:ring-claret/20"
              aria-label="Search specialties"
            />
            <Search className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-ink/40" strokeWidth={1.8} />
          </div>
        </div>
        ...
        {visible.length === 0 && (
          <p className="mt-8 text-center text-ink/55">No specialties match your search.</p>
        )}
      </div>
    </section>
  )
